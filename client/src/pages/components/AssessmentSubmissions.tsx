...
// Define the types for assessment request data
...
const AssessmentSubmissionsPage = () => {
  ...
  const { data, isLoading, isError, error } = useQuery<ApiResponse>({
    queryKey: ['/api/admin/assessment-requests'],
  ...
  const downloadAsJson = (submission: AssessmentSubmission) => {
    const dataStr = JSON.stringify(submission, null, 2);
    const dataUri = 'data:application/json;charset=utf-8,'+ encodeURIComponent(dataStr);
    
    const exportFileDefaultName = `assessment_request_${submission.id}.json`;
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl mx-auto">
            ...
            <p className="text-muted-foreground">
              {viewMode === 'list' 
                ? 'View and manage security assessment requests from the website.' 
                : 'Detailed information about the selected assessment request.'}
            </p>
          </div>
        </div>
      </section>
              ...
              <>
                {isLoading ? (
                  ...
                  <Card className="bg-destructive/10 border-destructive">
                    <CardHeader>
                      <CardTitle>Error Loading Assessment Requests</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <p>There was an error loading the assessment requests.</p>
                  ...
                  <div className="space-y-6">
                    {data?.submissions && data.submissions.length > 0 ? (
                          ...
                          <CardHeader>
                            <div className="flex justify-between items-start">
                              ...
                              <Badge variant="outline" className="text-xs">
                                {submission.data.industry || 'Unspecified Industry'}
                              </Badge>
                            </div>
                          </CardHeader>
                          <CardContent>
                      ...
                      <Card className="bg-card/30 backdrop-blur-sm border border-[hsl(var(--secondary))]/10">
                        <CardHeader>
                          <CardTitle>No Assessment Requests Yet</CardTitle>
                        </CardHeader>
                        <CardContent>
                          <p>There are no security assessment requests to display.</p>
                        </CardContent>
                        <CardFooter>
                          <Link href="/">
              ...
              selectedSubmission && (
                <Card className="bg-card/50 backdrop-blur-sm border border-[hsl(var(--secondary))]/10">
                  ...
                  <CardContent className="space-y-6">
                    ...
                    {selectedSubmission.data.industry && (
                      <div>
                        <h3 className="text-sm font-medium text-muted-foreground mb-1">Industry</h3>
                        <Badge variant="outline">{selectedSubmission.data.industry}</Badge>
                      </div>
                    )}
                    
                    <div>
                      <h3 className="text-sm font-medium text-muted-foreground mb-1">Assessment Type</h3>
                      <p>{selectedSubmission.data.service || 'Security Assessment'}</p>
                    </div>
